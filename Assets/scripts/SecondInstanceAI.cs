...
public class SecondInstanceAI : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (time > 10 && AIcount < 12) {
			skel = !skel;
			if (skel) {
				GameObject skel1 = Instantiate (SkeletonAI, new Vector3 (Random.Range (-10, 100), 0, 0), transform.rotation);
				Instantiate(SkeletonAI, new Vector3(-5, 0, 0), transform.rotation);
			}
			else if (!skel) {
				GameObject Orc1 = Instantiate (OrcAI, new Vector3 (Random.Range (-10, 100), 0, 0), transform.rotation);
